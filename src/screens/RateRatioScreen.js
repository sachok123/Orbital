...
export default function RateRatioScreen({ navigation }) {
    ...
    React.useEffect(() => {// rendering of question according to user level 
      let mounted = true; 
      firebase.database().ref(`rateandratio/${levels}`)
      .on('value', (qn) => {
        if (mounted){
    ...
    const onSubmitPressed = () => {
      ...
      if (correct == true && levels < 20) {
        setLevels(levels + 1)
        firebase.database().ref("users").child(users).child("rlevel").set(levels + 1) //update of user 4 ops level 
